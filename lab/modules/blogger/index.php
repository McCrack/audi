...
<html>
	<head>
		...
		<script src="/xhr/wordlist?d[0]=base&d[1]=modules&d[2]=blogger" defer charset="utf-8"></script>
				...
				<div class="tabs">
					...
					<form id="metadata" class="tab body-bg light-txt" autocomplete="off">
						...
						<div class="black-bg" align="right">
							<?php
							$category = $mySQL->getRow("SELECT name FROM gb_sitemap WHERE MaterialID = 9 AND language LIKE {str}", $language)['name'];
							$categories = $mySQL->get("SELECT name FROM gb_sitemap WHERE parent LIKE {str} AND language LIKE {str}", $category,$language);
							if(count($categories)):?>
						...
						(function(form){
							form.onsubmit=function(event){
								event.preventDefault();
								var box = new Box('{}', "blogger/savebox", function(){	box.drop(); });
