...
<html>
	...
	<body>
		...
		<div id="wrapper">
			...
			$list = $mySQL->get("SELECT SQL_CALC_FOUND_ROWS * FROM gb_pages CROSS JOIN gb_sitemap USING(PageID) CROSS JOIN gb_static USING(PageID) ORDER BY language,PageID DESC LIMIT {int}, {int}", ($page-1)*$limit, $limit);
					...
					<table cellspacing="0" cellpadding="10" rules="cols" bordercolor="#EEE">
						<thead>
							<tr class="light-btn-bg">
								<th>Language / name</th>
								<th colspan="2"><span data-translate="textContent">to menu</span> <span class="black-bg">cnt</span></th>
								<th colspan="2">Title <span class="black-bg">cnt</span></th>
								<th colspan="2">Description <span class="black-bg">cnt</span></th>
						...
						<tbody>
							...
							<tr data-id="<?=$row['PageID']?>">
								...
								<td contenteditable="true" class="header"><?=$row['header']?></td>
								<td class="v-bar-bg"><?=iconv_strlen($row['header'])?></td>
								<td contenteditable="true" class="title"><?=$row['title']?></td>
								<td class="v-bar-bg"><?=iconv_strlen($row['title'])?></td>
								<td contenteditable="true" class="description"><?=$row['description']?></td>
								<td class="v-bar-bg"><?=iconv_strlen($row['description'])?></td>
								<th><?=$row['views']?></th>
							...
							(function(tbody){
								tbody.querySelectorAll("td").forEach(function(cell){
									cell.onfocus=function(){
										...
										if(isNaN(["header","title","description"].inArray(cell.className))) return false
											...
											XHR.push({
												...
												onsuccess:function(response){
													if(parseInt(response)){
														...
														cell.nextElementSibling.textContent = cell.textContent.length;
														box.drop();
													}
												}
											});
