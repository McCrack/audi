...
<div id="<?=$handle?>" class="mount" style="width:740px">
	...
	<form class="box discount-box white-bg" autocomplete="off">
		...
		<div class="box-footer light-btn-bg" align="right">
			...
			<button type="reset" class="dark-btn-bg" data-translate="textContent" name="reset">reset</button>
		...
		(function(form){
			...
			form.reset.onclick=function(event){
				event.preventDefault();
				XHR.push({
					addressee:"/showcase/actions/rs_discount/<?=$HTTP_RAW_POST_DATA?>",
					onsuccess:function(response){
						if(parseInt(response)){
							var properties = document.querySelector("form#properties");

							properties.discount.value = "";
							properties.discount.dataset.id = "";
			...
			form.addbutton.onchange=function(){
				XHR.push({
					addressee:"/showcase/actions/ad_discount",
					onsuccess:function(response){
						if(parseInt(response)){
			...
			form.oninput=function(event){
				...
				event.target.timeout = setTimeout(function(){
					...
					XHR.push({
						addressee:"/showcase/actions/ch_discount/"+form.DiscountID.value+"/"+event.target.name,
						body:val,
						onsuccess:function(response){
			...
			form.init=function(){
				...
				tbody.querySelectorAll("tr").forEach(function(row){
					row.onclick=function(){
						XHR.push({
							addressee:"/showcase/actions/gt_discount/"+row.dataset.id,
							onsuccess:function(response){
				...
				tbody.querySelectorAll("tr>th.drop-row").forEach(function(cell){
					cell.onclick=function(){
						...
						XHR.push({
							addressee:"/showcase/actions/dl_discount/"+row.dataset.id,
							onsuccess:function(response){
