...
<html>
	<head>
		...
		<script src="/xhr/wordlist?d[0]=base&d[1]=modules&d[2]=uploader" defer charset="utf-8"></script>
	</head>
	<body>
		...
		<main>
			...
			<form id="folder" class="white-bg" autocomplete="off">
				...
				<label data-path="<?=($_GET['path'].'/'.$path['basename'])?>" class="file" data-type="<?=$type[1]?>">
					<input type="<?=$multiselect?>" name="files-on-folder" data-type="<?=$type[0]?>" value="<?=($config->{$_GET['subdomain']}."/data".$_GET['path']."/".$path['basename'])?>" hidden>
					<div>
						<img src="<?=($config->{$_GET['subdomain']}."/data".$_GET['path']."/".$path['basename']."#".$hash)?>" alt="<?=$symbol?>">
						<?=$path['basename']?>
					</div>
				</label>
				<?endif; endforeach?>
				...
				folder.querySelectorAll("label").forEach(function(node){
					node.ondblclick=function(){
						if(node.classList.contains("folder")){
							...
							location.href = "/navigator/actions/download?path=<?=$_GET['subdomain']?>/"+node.dataset.path;
						}
					}
				...
				var rename = function(obj){
					...
					let box = promptBox("", function(form){
						...
						XHR.push({
							addressee:"/navigator/actions/rename",
							body:JSON.stringify({
								old:"<?=$_GET['subdomain']?>/"+obj.dataset.path,
								new:"<?=$_GET['subdomain']?>/"+newpath
							}),
				...
				var zip = function(obj){
					XHR.push({
						addressee:"/navigator/actions/create-zip",
						body:"<?=$_GET['subdomain']?>/"+obj.dataset.path,
				...
				var unzip = function(obj){
					XHR.push({
						addressee:"/navigator/actions/unzip",
						body:"<?=$_GET['subdomain']?>/"+obj.dataset.path,
						onsuccess:function(response){
							location.reload();
						}
				...
				var paste = function(){
					if(STANDBY.movelist.length){
						var addressee = "/navigator/actions/move?path=<?=$_GET['subdomain']?>/data<?=$_GET['path']?>";
						var body = JSON.encode(STANDBY.movelist);
					}else if(STANDBY.copylist.length){
						var addressee = "/navigator/actions/copy?path=<?=$_GET['subdomain']?>/data<?=$_GET['path']?>";
				...
				var upload = function(){
					...
					inp.onchange = function(){
						XHR.uploader(inp.files, "/navigator/actions/upload?path=<?=$_GET['subdomain']?>/<?=$_GET['path']?>", function(response){
							location.reload();
						});
					}
					inp.click();
				...
				var createFolder = function(){
					promptBox("new folder name",function(form){
						XHR.push({
							addressee:"/navigator/actions/create-folder",
							body:JSON.encode({
								name:form.field.value.trim().translite(),
								path:"<?=$_GET['subdomain']?>/<?=$_GET['path']?>"
							}),
							onsuccess:function(response){
								location.reload();
							}
				...
				var remove = function(obj){
					...
					if(lst.length){
						confirmBox("delete selected",function(){
							XHR.push({
								addressee:"/navigator/actions/delete",
				...
				var prowler = function(){
					new Box("<?=$_GET['subdomain']?>/<?=$_GET['path']?>", "navigator/prowlerbox");
				}
				var createMediaset = function(){
					var box = new Box(null,"mediaset/createbox",function(form){
						...
						XHR.push({
							addressee:"/mediaset/actions/create-mediaset",
							body:JSON.encode({
			...
			<aside>
				<form id="tree" class="root light-txt" autocomplete="off">
					<?foreach(glob("../*", GLOB_ONLYDIR) as $i=>$subdomain): $subdomain=basename($subdomain)?>
