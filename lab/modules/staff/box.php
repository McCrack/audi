	...
	<form class="box staff-box light-btn-bg" autocomplete="off">
						...
						(function(select){
							select.onchange=function(){
								if(select.value) {XHR.push({
									addressee:"/staff/actions/load-user/"+select.value,
		...
		(function(form){
			...
			form.remove.onclick=function(event){
				...
				alert("remove");
				//XHR.push({addressee:"/staff/actions/delete/"+(form.uid.value || 0)});
			}
			form.onsubmit=function(){
				XHR.push({
					addressee:"/staff/actions/save/"+(form.uid.value || 0),
					body:JSON.encode({
			...
			form.remove.onclick=function(event){
				event.preventDefault();
				XHR.push({
					addressee:"/staff/actions/delete/"+(form.uid.value || 0),
