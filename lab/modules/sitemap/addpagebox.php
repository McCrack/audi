...
<div id="<?=$handle?>" class="mount modal">
	...
	.addpage-box>div.box-body>fieldset>div.select>select{
		max-width:160px;
	}
	.addpage-box>div.box-body>fieldset>input,
			...
			<fieldset><legend data-translate="textContent">category</legend>
				<div class="select">
					<select name="parent">
						...
						<?$categories=$mySQL->getGroup("SELECT name FROM gb_sitemap CROSS JOIN gb_pages USING(PageID) WHERE type LIKE 'category' AND language LIKE {str}", $cng->language)['name'];
