...
if(defined("ARG_2")){
	...
	WHERE 
		...
		language LIKE {str}
	LIMIT 1", ARG_2, ARG_1);
	define("PAGE_ID", $material['PageID']);
	...
	<body>
		...
		<div id="wrapper">
					...
					<div id="sitemap" class="tab body-bg light-txt">
						...
						<?function staticTree(&$items, $offset="root"){
							...
							<div class="root">
								...
								<a data-id="<?=$val['PageID']?>" data-parent="<?=$val['parent']?>" href="/sitemap/<?=($val['language'].'/'.$val['name'])?>" class="<?if($val['PageID']==PAGE_ID):?>active-txt<?elseif($val['published']==='Published'):?>published-txt<?endif?>"><?=(empty($val['header'])?$val['name']:$val['header'])?> <label>❬</label></a>
			...
			<header class="h-bar t light-txt">
				<?if(PAGE_ID):?><a class="tool" title="Back" href="/sitemap/<?=ARG_1?>">❬</a><?endif?>
				<div class="toolbar t">
					...
					<?if(defined("ARG_2")):?>
					<label title="clone page" data-translate="title" class="tool" onclick="new Box(null, 'sitemap/clonepagebox/<?=PAGE_ID?>')">&#xe925;</label>
			...
			<section>
				<div class="tabs">
					...
					<form id="metadata" class="tab body-bg light-txt" autocomplete="off">
						<div class="h-bar light-btn-bg">
							...
							<small>ID: <output name="MaterialID"><?=$material['MaterialID']?></output>/<output name="PageID"><?=PAGE_ID?></output></small>
							<div class="select" title="<?=$material['MaterialID']?>">
								<select name="language" class="active-txt">
									<?$pgs = $mySQL->get("SELECT language,name FROM gb_sitemap WHERE MaterialID={int}", $material['MaterialID']);
									foreach($pgs as $pg):?>
									<option <?if($pg['language']==$material['language']):?>selected<?endif?> value="<?=$pg['language']?>/<?=$pg['name']?>"><?=$pg['language']?></option>
									<?endforeach?>
									<script>
									(function(select){
										select.onchange=function(){
											location.pathname = "sitemap/"+select.value;
										}
									})(document.currentScript.parentNode)
									</script>
								</select>
						...
						(function(form){
							...
							form.onreset=function(event){
								...
								confirmBox("remove material", function(){
									XHR.push({
										addressee:"/sitemap/actions/remove-page/<?=PAGE_ID?>", 
										onsuccess:function(response){
											setTimeout(function(){
				...
				<form id="rightbar-tabs" class="v-bar r v-bar-bg" data-default="right-default" autocomplete="off">
					...
					<div class="toolbar">
						<label title="microdata" class="tool" data-translate="title" onclick="new Box('<?=PAGE_ID?>','sitemap/microdatabox')">&#xe8ab;</label>
