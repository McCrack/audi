...
<div id="<?=$handle?>" class="mount" style="width:820px;">
	...
	<form class="box pattern-box white-bg">
		...
		<div class="box-caption active-bg">HTML<?include_once("components/movebox.php")?></div>
		<div class="h-bar active-bg">
			<input name="path" value="patterns/html" required readonly>
		...
		(function(form){
			...
			form['create-folder'].onclick=function(event){
				...
				promptBox("new folder name", function(promptForm){
					XHR.push({
						addressee:"/patterns/actions/create-folder/html",
					...
					itm.onclick=function(){
						XHR.push({
							...
							onsuccess:function(response){
								...
								form.pname.value = itm.dataset.name.split(/\./).shift();
...
<?php
function patterns_tree($root="patterns/html", $level=0){
