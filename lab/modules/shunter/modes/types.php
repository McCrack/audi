...
$type = "";
$types = preg_split("/\s*,+\s*/", $mySQL->settings['shunter']['hidden types'], -1, PREG_SPLIT_NO_EMPTY);
$types[] = "hidden";

if(in_array(USER_GROUP, $privileged)){
	$where = $mySQL->parse("type NOT IN ({arr})",$types);
}else $where = $mySQL->parse(
	"type NOT IN ({arr}) AND (CommunityID IS NULL OR CommunityID = {int})",
	$types,
	COMMUNITY_ID
);
