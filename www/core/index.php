...
class Data{
	public $SiteName,$logo,$canonical,$root,$title;
	...
	public function __construct($list){
		...
		if($GLOBALS['mySQL']->status['affected_rows']>0){
			define("PAGE_ID", $id);
			$post = $GLOBALS['mySQL']->getRow("SELECT * FROM gb_pages CROSS JOIN gb_blogfeed USING(PageID) CROSS JOIN gb_blogcontent USING(PageID) WHERE PageID={int} LIMIT 1", $id);
			if($GLOBALS['mySQL']->status['affected_rows']>0){
	...
	private function buildScheme(&$obj){
		foreach($obj as $key=>&$itm){
			...
			}elseif( preg_match("@^md:(.*)@i", $itm, $matches) ) $itm = $this->{$matches[1]};
		}
	}
	/*~~~~~~~*/
